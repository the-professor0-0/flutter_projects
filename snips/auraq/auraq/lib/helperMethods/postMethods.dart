...
void uploadFile(context,token,filePath,String title,String page, String language,var user) async {
  ...
  try {
    ...
    var response = await dio.post(mainUrl+"/api/publishbook", data: formData);
...
void reUploadFile(context,token,filePath,var book,var user) async {
  print("Uploading");
  String fileName = basename(filePath.path);
  try {
    var dio = Dio();
    ...
    var response = await dio.post(mainUrl+"/api/updatebook", data: formData);
    var res=response.data.toString();
    print(res);
    if(res=="{messsage: Your book successfully published, status_code: 200}"){
      Fluttertoast.showToast(
...
void uploadInvoice(context,token,filePath,var book,var user) async {
  String fileName = basename(filePath.path);
  try {
    print(user["data"]["id"]);
    var dio = Dio();
